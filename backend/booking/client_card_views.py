...
class ProviderClientCardView(APIView):
    ...
    def _get_card(self, request):
        if request.user.role not in (User.Role.PROVIDER, User.Role.STAFF):
            return None, None, Response({"detail": "Нет доступа"}, status=403)
        provider = _provider_for_user(request.user)
        if not provider:
            return None, None, Response({"detail": "Организация не найдена"}, status=403)
        raw = request.query_params.get("client") or request.data.get("client")
        client = _resolve_client(str(raw or ""))
        if not client:
            return None, None, Response({"detail": "Клиент не найден"}, status=404)
        card, _ = ProviderClientCard.objects.get_or_create(provider=provider, client=client)
        return provider, card, None

    def _serialize(self, card, provider):
        recent = (
    ...
    def get(self, request):
        _provider, card, err = self._get_card(request)
        if err:
            return err
        return Response(self._serialize(card, _provider))
