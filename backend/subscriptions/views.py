...
class YooKassaWebhookView(APIView):
    ...
    def post(self, request):
        ...
        # Prefer explicit metadata, then fall back by payment id (missing/wrong meta).
        if typ == "booking" or meta.get("booking_id"):
            if _mark_booking_paid_by_payment_id(yk_id, meta):
                return Response({"detail": "ok"})
        if typ == "cafe_order":
            if _mark_cafe_paid_by_payment_id(yk_id, meta):
                return Response({"detail": "ok"})
        if typ == "shop_order":
