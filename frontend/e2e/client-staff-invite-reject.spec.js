...
test.describe("Client staff invite reject", () => {
  test("/cabinet → Отклонить → POST reject-invite", async ({ page }) => {
    ...
    let rejected = false;
    page.on("request", (req) => {
      if (req.method() === "POST" && req.url().includes(`/booking/staff/${INVITE.id}/reject-invite`)) {
        rejected = true;
      }
    });
    ...
    await expect.poll(() => rejected, { timeout: 15_000 }).toBe(true);
