...
const PLATFORM_APPS = [
  ...
  },
  {
    ...
    accent: "#6a4c93",
    title: "Вмагазине",
    lead: "Клиентский сервис магазинов Вместе: поиск витрин рядом, избранное и история ваших заказов с самовывозом и доставкой.",
    highlights: [
      "Поиск магазинов по названию и адресу",
      "Сортировка по близости, если разрешена геолокация",
      "Избранные магазины в один клик",
      "Мои заказы: статус, сумма и переход на витрину",
      "Открытие публичной витрины /s/… без отдельной регистрации",
    ],
    categories: [
      { name: "Поиск", items: ["Магазины сферы «Магазины»", "Витрины салонов и автосервисов", "Адрес и рейтинг"] },
      { name: "Покупки", items: ["Избранное", "История заказов", "Ссылка на витрину"] },
      { name: "Доступ", items: ["Раздел «Сервисы» в кабинете", "Один аккаунт Вместе", "Работает в браузере"] },
    ],
    cta: { label: "Открыть в кабинете", href: "/services" },
  },
  {
...
const FAQ = [
  {
    ...
    answer:
      "Сферы (салон, кафе, магазины, маркетплейсы) — это готовые кабинеты под тип организации. Сервисы — отдельные приложения платформы, которые подключаются поверх аккаунта Вместе.",
  },
  {
    question: "Нужна отдельная регистрация для Вменю или Вмагазине?",
    answer: "Нет. Войдите в Вместе и откройте раздел «Сервисы».",
  },
  {
    question: "Кто может пользоваться Вменю и Вмагазине?",
...
export default function PlatformAppsPage() {
  useEffect(() => {
    setPageMeta({
      title: "Сервисы платформы — Вменю, Вмагазине и голосовой ассистент | Вместе",
      description:
        "Микросервисы Вместе: Вменю (рецепты), Вмагазине (магазины и заказы) и голосовой ассистент. Подробности по каждому сервису.",
      path: "/apps",
    });
  }, []);
