...
| `provider-bookings.spec.js` | confirm / no-show / arrived / mark-done / cancel-by-org |
...
Backend also covers create booking → waitlist `BOOKED`, client cancel → package restore + waitlist notify, org confirm/no-show/arrived/done/cancel-by-org HTTP, subscription promo/cancel.
