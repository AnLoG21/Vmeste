...
## Виджет домашнего экрана (записи)

На Android есть виджет **«Ближайшая запись и клиенты на сегодня»**.

- Показывает: «Сегодня N клиентов» (у клиента — «N записей») и ближайшую будущую запись.
- Данные пишет JS после загрузки `/api/booking/` в SharedPreferences (токен WebView виджету недоступен).
- Тап открывает `vmeste://app/bookings`.
- Код: `BookingWidgetProvider` / `BookingWidgetPlugin`, `frontend/src/bookingWidget.js`.

Чтобы виджет появился: пересобрать APK (`npm run cap:sync` + Gradle), установить, **войти в аккаунт**, затем на рабочем столе: виджеты → Вместе.
